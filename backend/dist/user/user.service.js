...
let UserService = class UserService {
    ...
    async githubCallback(user) {
        ...
        if (!email) {
            throw new common_1.UnauthorizedException("No email provided by GitHub account.");
        }
        try {
            ...
            if (!dbUser) {
                throw new common_1.InternalServerErrorException("Failed to create or find GitHub user.");
            }
            ...
            if (!dbUser) {
                throw new common_1.InternalServerErrorException("Failed to update GitHub user verification.");
            }
            const payload = {
                sub: dbUser._id,
                username: user.username
            ...
            console.log(error);
            throw new common_1.InternalServerErrorException("Authentication via GitHub failed.");
        }
    }
};
